...
#            self.__user = user
        ...
        * アカウントと時刻ごとに異なるOTP(未実装)
        ...
        def get(self, http_method, endpoint):
            ...
            params = {}
            account = self.__db.Accounts['Accounts'].find_one(Username=self.__authData.Username)
#            account = self.__db.Accounts['Accounts'].find_one(Username=self.__user.Name)
...
#                two_factor = self.__db.Accounts['TwoFactors'].find(AccountId=account['Id'])
